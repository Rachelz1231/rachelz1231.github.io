...
function mergeWithFallback(orcidItems) {
  // Index ORCID results by normalized title.
  const orcidByTitle = new Map(
    orcidItems.map((p) => [normalizeTitle(p.title), p])
  );

  // Enrich ORCID entries with editorial notes / fill missing fields from fallback.
  const enriched = orcidItems.map((p) => {
    const f = fallbackPublications.find(
      (x) => normalizeTitle(x.title) === normalizeTitle(p.title)
    );
    if (!f) return p;
    return {
      ...p,
  ...
  // Append fallback papers that ORCID doesn't have yet.
  const extras = fallbackPublications.filter(
    (f) => !orcidByTitle.has(normalizeTitle(f.title))
  );

  return [...enriched, ...extras].sort((a, b) => (b.year || 0) - (a.year || 0));
}
