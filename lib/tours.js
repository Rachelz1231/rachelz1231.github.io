...
// 把各天的 legs 按顺序串成一条完整路线，供路线图使用。
// 返回 { stops, legs }，stops 比 legs 多一个（起点）。
export function routeOf(tour) {
  const legs = tour.days.flatMap((day) => day.legs ?? []);
  if (!legs.length) return null;
  return { stops: [legs[0].from, ...legs.map((leg) => leg.to)], legs };
}
